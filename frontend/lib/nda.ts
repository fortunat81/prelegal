...
export interface StandardTermsClause {
  number: number;
  title: string;
  body: (data: NdaFormData) => string;
}

export const standardTermsClauses: StandardTermsClause[] = [
  {
    number: 1,
    title: "Introduction",
    body: (d) =>
  ...
  },
  {
    ...
    body: (d) =>
      `This MNDA commences on the ${formatDate(d.effectiveDate)} and expires at the end of the MNDA Term (${mndaTermText(d)}). Either party may terminate this MNDA for any or no reason upon written notice to the other party. The Receiving Party's obligations relating to Confidential Information will survive for the Term of Confidentiality (${confidentialityTermText(d)}), despite any expiration or termination of this MNDA.`,
  },
  {
    number: 6,
